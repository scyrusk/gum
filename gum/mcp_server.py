...
# The retrieval + pseudonymization that answers `gather_context` lives in the
# shared, FastMCP-free `context` module so the execution bridge grounds a
# dispatched task on the *same* assembly (spec #4: don't fork a second grounding
# path). This tool is now a thin adapter over it. `_focus_terms` is re-exported
# here for the tests that already import it from this module.
from .context import _focus_terms, gather_context as _gather_context
from .gum import gum
...
def build_mcp(gum_instance: gum, *, sanitize: bool = True) -> FastMCP:
    ...
    async def gather_context(topic: str, limit: int = 10) -> dict[str, Any]:
        # Thin adapter: the retrieval + egress pseudonymization live in the
        # shared context module so the execution bridge reuses the exact same
        # assembly (spec #4).
        return await _gather_context(
            gum_instance, topic, sanitizer=sanitizer, limit=limit
        )
