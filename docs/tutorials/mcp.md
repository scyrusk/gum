...
It exposes six tools: `gather_context(topic, limit)` (relevance-ranked propositions for a task), `recent_context(limit)` (a snapshot of recent activity), `inspect_proposition(proposition_id, limit)` (the raw observations backing a proposition, so the agent can ground a draft in the underlying evidence rather than the one-line summary), `agenda(limit, window_days)` (the commitment & deadline radar, the same one behind `gum agenda` — the user's open commitments ranked most-urgent-first, so the agent knows what's time-critical before it schedules, drafts, or plans; pass `window_days` to only include deadlines within that horizon, with overdue and undated items always kept; its response also carries a `today` field — the user's real local date and weekday — so an off-device agent can turn each commitment's absolute `due_date` into "what's due today/this week" instead of guessing the date from its own stale cutoff), and `upcoming_deadlines(window_days, limit)` (a **deterministic** date scan that returns the raw set of propositions whose text carries an absolute `YYYY-MM-DD` deadline — no model extraction, so unlike `agenda` it surfaces the *complete* dated signal for a frontier agent to reason over itself and to catch dated items the local extractor dropped or merged; each item is a proposition plus its parsed `deadline` and `days_until_due`, overdue always kept, upcoming within `window_days`, soonest-first, and it echoes the same `today` anchor), and `add_agenda_item(title, due_date, status, note)` (the one **write** tool — put a commitment on the user's agenda on their behalf after helping them plan or commit to something; provide an absolute `YYYY-MM-DD` `due_date` resolved against the `today` anchor, and keep any pseudo-IDs in the `title`/`note` verbatim — the GUM rehydrates them to the real values on-device before saving, so the item lands in the user's local agenda in real terms without the names ever coming back to you, and the tool returns only the new `item_id` and a count of restored placeholders). `gather_context` accepts a whole task instruction as its `topic` — it strips instruction verbs and stopwords (e.g. *"**draft** a grant proposal **for the** …"*) so the search runs on the substantive terms and doesn't drag in propositions that merely share a common word; the terms actually searched are echoed back in the response's `search_terms` field. Because the returned propositions are pseudonymized, an entity you named in the task (e.g. *"Schmidt"*) appears in the context under a pseudo-ID (e.g. `[ORG_1]`); `gather_context` therefore also returns a `query_aliases` map (real name → pseudo-ID) for the entities in your topic, so the agent can tell which pseudonymized propositions actually concern the thing it was asked about. This exposes nothing new — the values come from the agent's own task — it just bridges the task to the sanitized context. Sanitization requires the extra (`uv sync --extra sanitize` in a checkout, or `uv pip install 'gum-ai[sanitize]'`); for a fully-local, trusted agent you can serve raw propositions with `gum mcp --no-sanitize`.
...
A second **prompt**, `daily_agenda(horizon)`, packages a related workflow for a *morning briefing*: it drives the agent to pull the deadline radar (reading its `today` anchor as "now"), cross-check it against `upcoming_deadlines`' raw dated propositions (reconciling by proposition `id`), and add recent context, then synthesize a prioritized, date-grounded agenda — overdue and due-today items first, then upcoming deadlines, then high-confidence undated commitments. This is the "let a capable frontier agent build my agenda" path: the local model behind `gum agenda` extracts and ranks commitments adequately, but Claude — handed the same pseudonymized propositions with their absolute `YYYY-MM-DD` deadlines and the `today` anchor — can prioritize and phrase the day far better, all without any raw identity leaving the machine.
